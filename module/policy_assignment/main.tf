...
locals {
  # modify the syntax: instead of param_name = paramvalue, we need param_name = { value = param_value }
  # this can be removed once we can don't reload the policy_assginment files anymore
  parameter_to_update = { for policy_assignment, params in var.alz_archetype.policy_assignments_to_modify :
    policy_assignment => {
      for param_name, param_value in jsondecode(params["parameters"]) :
...
resource "azurerm_management_group_policy_assignment" "this" {
  ...
  # we update the parameters with the policy_assignments_to_modify parameters
  parameters = try(each.value.properties.parameters, null) != null && try(each.value.properties.parameters, {}) != {} ? jsonencode(merge(each.value.properties.parameters, try(local.parameter_to_update[each.key], {}))) : null
