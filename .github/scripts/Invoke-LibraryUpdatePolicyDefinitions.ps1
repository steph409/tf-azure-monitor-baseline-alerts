...
param (
    ...
    [Parameter()][String]$TargetPath = "$PWD/baseline_alert",
    [Parameter()][String]$SourcePath = "$PWD/azure-monitor-baseline-alerts",
...
$policyDefinitionFilePaths = (
    Get-ChildItem -Recurse:$true -Path "$SourcePath/services/*" `
        -File `
        -Include "*.json" `
        -Exclude "*.AzureChinaCloud.json", "*.AzureUSGovernment.json"
).FullName
$policySetDefinitionFilePaths = (
    Get-ChildItem -Path "$SourcePath/patterns/alz/policySetDefinitions/*" `
        -File `
        -Include "*.json" `
        -Exclude "*.AzureChinaCloud.json", "*.AzureUSGovernment.json"
).FullName
